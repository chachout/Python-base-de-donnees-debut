...
cur=conn.cursor()
print("Bonjour bienvenue dans la base de donnée musique")
N=int(input("Combien de titre voulez-vous rentrer : "))
a=0
while a<N :
    T=input("Mettez le titre de la chanson : ")
    C=input("Mettez le nom du compositeur : ")
    D=int(input("Mettez la durée de la chanson : "))
    I=input("Mettez le nom de l'interprète : ")
    data =[(T,C,D,I)]
    for tu in data :
        cur.execute("INSERT INTO oeuvres(titre,comp,duree,interpr) Values(?,?,?,?)",tu)
    a=a+1
conn.commit()
conn.close()